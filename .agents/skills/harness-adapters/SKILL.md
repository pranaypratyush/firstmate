...
If such a dialog is showing, accept it from an active firstmate session using `bin/fm-send.sh --home <absolute-firstmate-home> <window> --key Enter`, or the choice the dialog requires, then verify the brief started processing.
