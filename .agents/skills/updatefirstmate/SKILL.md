...
3. **Nudge each updated live secondmate.**
   ...
   ```sh
   bin/fm-send.sh --home <absolute-firstmate-home> <id> 'firstmate was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.'
   ```
   Always pass the active home through `--home` so the helper remains the command prefix and the steer cannot resolve through ambient state.
