...
test_help_documents_home_argument_contract() {
  ...
  help=$(cat "$out")
  assert_contains "$help" "fm-send.sh --home <absolute-firstmate-home>" "help should show the approval-prefix-shaped invocation"
  assert_contains "$help" "--home must be the first argument" "help should define the approval-prefix argument position"
  assert_contains "$help" "existing absolute home with a state directory" "help should define home validation"
  assert_contains "$help" "takes precedence over FM_HOME and operational-directory overrides" "help should define argument precedence"
  assert_contains "$help" "never falls back to the repo root" "help should retain the no-ambient-fallback contract"
