...
Payload text and handled acknowledgement remain in that inbox, while the loaded extension refuses an exact-session request before notification because OMP exposes no session-addressed `sendMessage(..., { triggerTurn: true })` boundary; neither composer text nor Enter transports or proves the request.
