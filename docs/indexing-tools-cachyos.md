...
The plan validates CachyOS/Arch, x86-64 or arm64, the exact Git root, local-filesystem suitability, binary/package ownership, PATH precedence, Ollama service provenance, loopback binding, accelerator prerequisites, existing config/index layout, detected languages, RAM, disk, CPU/load, and the grepai v0.35.0 scanner census before mutation.
...
Prepare five project-specific benchmark queries and include every detected Serena language before changing the selected action from plan to apply.
...
Symlink-managed lazy-mcp configs or hierarchy roots are refused without changing their targets.
