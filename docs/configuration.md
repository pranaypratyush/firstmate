...
Its cross-home durable ownership record binds backend-specific endpoint generations for collision-free same-task recovery, and Herdr uses a token-bound forward-only endpoint journal so interrupted retries resume one exact response-derived workspace, tab, pane, and native agent identity or fail closed without duplication.
