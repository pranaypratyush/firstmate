...
This recovery-oriented path is intentionally limited to tmux and to adapters whose spawn wiring does not write into the checkout; it requires a named, registered worktree of the requested project, refuses durable or live task claims and ambiguous live-task inventory, and preserves the current branch, HEAD, index, working files, and external lease.
Task metadata records `worktree_ownership=adopted` plus the intake branch and HEAD, and `fm-teardown.sh` applies normal report and landed-work gates while removing only the endpoint and task state, never the adopted worktree or branch.
Same-task recovery preserves later durable task fields, keeps metadata and the generated worker addendum aligned across refusals, and removes a partially launched endpoint so the recorded task can be retried without a window collision.
Secondmate retirement refuses before mutation while any adopted descendant remains, because removing that home could delete the worktree or its Git common directory even when the worktree path is outside the home.
