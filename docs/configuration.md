...
This recovery-oriented path is intentionally limited to tmux and to adapters whose spawn wiring does not write into the checkout; it requires a named, registered worktree of the requested project's canonical primary checkout, refuses durable claims across every Firstmate home plus live task claims and ambiguous live-task inventory, and preserves the current branch, HEAD, index, working files, and external lease.
Its durable ownership record binds any created endpoint generation for collision-free same-task recovery, and `fm-teardown.sh` serializes against that recovery while applying normal report and landed-work gates and retiring only Firstmate task state and endpoints, never the adopted worktree or branch.
