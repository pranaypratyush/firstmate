...
An ordinary ship or scout that must continue a pre-existing, externally leased Git worktree can pass `fm-spawn.sh --existing-worktree <absolute-path>` instead of allocating another copy.
This recovery-oriented path supports tmux and Herdr with adoption-safe harnesses whose spawn wiring does not write into the checkout; it requires a named, registered worktree of the requested project's canonical primary checkout, refuses durable claims across every Firstmate home plus live task claims and ambiguous live-task inventory, and preserves the current branch, HEAD, index, working files, and external lease.
Herdr adoption additionally requires the launcher's current pane, named-session socket, and parent workspace to resolve from authoritative runtime identity, so an outside-Herdr caller, duplicate running session identity, mutable workspace label, ambient session variable, or visually focused pane can never choose placement.
Its cross-home durable ownership record binds backend-specific endpoint generations for collision-free same-task recovery, and Herdr uses a token-bound forward-only endpoint journal bound to the original branch, HEAD, and immutable brief digest.
The journal commits launch submission before the first launch byte, so interrupted retries resume one exact response-derived workspace, tab, pane, and native agent identity or fail closed without launch resubmission or duplication.
Task metadata is published only after that Herdr identity and the exact adopted CWD are complete and independently verified.
`fm-teardown.sh` serializes against recovery, applies normal report and landed-work gates, and retires only the exact Firstmate task endpoint and state, never the adopted worktree or branch.
Secondmate retirement refuses while a retained adopted descendant could be removed or lose its Git dependency, including after ordinary task-state cleanup.
The exact accepted modes, validation order, metadata fields, generated worker addendum, recovery reuse rules, and refusal messages are owned by the headers and `--help` output of [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) and [`bin/fm-teardown.sh`](../bin/fm-teardown.sh).
