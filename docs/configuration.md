...
This recovery-oriented path is intentionally limited to tmux and to adapters whose spawn wiring does not write into the checkout; it requires a named, registered worktree of the requested project, refuses an existing task claim, and preserves the current branch, HEAD, index, working files, and external lease.
