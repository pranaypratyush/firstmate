...
# "off" preferences propagate as files. Primary config/nm-live-view carries the
# same default-on no-mistakes companion opt-out. Primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces nm-live-view startup-memory-budget trace-context}"
