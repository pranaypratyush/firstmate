...
family_for_basename() {
  case "$1" in
    ...
    fm-indexing-tools-cachyos.test.sh|fm-kimi-harness.test.sh|fm-muse-harness.test.sh|fm-herdr-lab.test.sh|fm-lint.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-install-herdr.sh|bin/fm-install-treehouse.sh|bin/fm-herdr-ci-cleanup.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-lint.sh|bin/fm-install-shellcheck.sh|bin/fm-indexing-tools-cachyos.sh|\
