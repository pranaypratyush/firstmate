...
Include the exact line \`Firstmate-Validation-Generation: <plan-generation>\` in the No-Mistakes \`--intent\`, then bind the returned id with \`$FM_ROOT/bin/fm-receipt-check.sh $task_id --bind-run <run-id> --generation <plan-generation>\` so completion can prove that exact run, generation, path, and head. Installed supplied-intent logs that omit the exact generation are unprovable and cannot bind.
