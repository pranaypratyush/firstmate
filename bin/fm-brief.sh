...
Include the exact line \`Firstmate-Validation-Generation: <plan-generation>\` in the No-Mistakes \`--intent\`, then immediately attest the returned run id with \`$FM_ROOT/bin/fm-receipt-check.sh $task_id --attest-run <run-id> --generation <plan-generation>\` before binding it with \`$FM_ROOT/bin/fm-receipt-check.sh $task_id --bind-run <run-id> --generation <plan-generation>\` so completion can prove that exact run, generation, path, and head.
