...
Include the exact line \`Firstmate-Validation-Generation: <plan-generation>\` in the No-Mistakes \`--intent\`, then launch it through \`$FM_ROOT/bin/fm-receipt-check.sh $task_id --launch-run --generation <plan-generation> -- [axi-run-args...]\`; this records the returned run id with the plan at creation time. Bind that returned id with \`$FM_ROOT/bin/fm-receipt-check.sh $task_id --bind-run <run-id> --generation <plan-generation>\` so completion can prove that exact run, generation, path, and head.
