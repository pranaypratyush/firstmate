...
`bin/fm-send.sh --home <absolute-firstmate-home>` is the approval-friendly operator form and fails closed when neither that argument nor compatibility `FM_HOME` is explicit, so a steer cannot silently resolve against another home.
